...
def _WriteWorkspace(main_gyp, sources_gyp, params):
    ...
    workspace_path = f"{build_file_root}.xcworkspace"
    ...
    for gyp_name in [main_gyp, sources_gyp]:
        name = f"{os.path.splitext(os.path.basename(gyp_name))[0]}.xcodeproj"
        name = xml.sax.saxutils.quoteattr(f"group:{name}")
...
def _TargetFromSpec(old_spec, params):
    ...
    if ninja_toplevel:
        ...
        ] = f"{ninja_toplevel}/$(CONFIGURATION)$(EFFECTIVE_PLATFORM_NAME)"
    ...
    if ninja_toplevel:
        ninja_target["actions"] = [
            {
                "action_name": f"Compile and copy {target_name} via ninja",
                "inputs": [],
                "outputs": [],
                "action": [
                    "env",
                ...
                "message": f"Compile and copy {target_name} via ninja",
            }
        ]
...
def CreateWrapper(target_list, target_dicts, data, params):
  ...
  """
    ...
    sources_gyp = os.path.join(
        os.path.dirname(main_gyp), f"{sources_target_name}.gyp"
    )
